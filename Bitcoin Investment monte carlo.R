# Bitcoin Investment Risk Monte Carlo Simulation
...
# Historical Bitcoin volatility and return estimation
# Note: These are hypothetical parameters and should be updated with real data
annual_return <- 0.5  # 50% expected annual return
annual_volatility <- 0.8  # 80% annual volatility

# Monte Carlo Simulation Function
simulate_bitcoin_investment <- function(initial_investment, 
                                        annual_return, 
                                        annual_volatility, 
                                        days) {
...
ggplot(results_df, aes(x = final_value)) +
  ...
  labs(
    title = "Bitcoin Investment Monte Carlo Simulation",
...
save(results_df, final_value_stats, file = "bitcoin_investment_simulation.RData")